...
def check_amp(model):
    ...
    if device.type == "npu":  
        try:  
            ...
            # NPU 设备支持 AMP  
            LOGGER.info(f"{prefix}NPU device detected, enabling NPU AMP support")  
            return True  
        except ImportError:  
            LOGGER.warning(f"{prefix}torch_npu not available for NPU AMP")  
            return False  
    elif device.type in {"cpu", "mps"}:  
        return False  # AMP only used on CUDA/NPU devices  
