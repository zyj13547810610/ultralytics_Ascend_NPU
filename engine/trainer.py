...
class BaseTrainer:
    ...
    def _get_memory(self, fraction=False):
        ...
        elif self.device.type == "npu":  # 添加 NPU 支持  
            try:  
                import torch_npu  
                memory = torch.npu.memory_reserved()  
                if fraction:  
                    # NPU 设备属性获取方式可能不同，需要根据 torch_npu API 调整  
                    total = torch.npu.get_device_properties(self.device.index).total_memory  
            except Exception:  
                # 如果无法获取 NPU 内存信息，返回默认值  
                return 0 if fraction else 0  
