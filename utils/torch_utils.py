    ...
    if npu:  
        ...
        try:  
            ...
            if not torch.npu.is_available():  
                raise ValueError("NPU requested but torch.npu.is_available() returned False")  
              
            # 处理 NPU 设备索引，例如 'npu:0' -> '0'  
            npu_device = device.replace("npu:", "").replace("npu", "0")  
            if npu_device == "":  
                npu_device = "0"  
              
            # 验证 NPU 设备索引  
            npu_count = torch.npu.device_count()  
            if int(npu_device) >= npu_count:  
                raise ValueError(f"Invalid NPU device index {npu_device}. Available NPU devices: 0-{npu_count-1}")  
              
            s += f"NPU:{npu_device} (NPU available, {npu_count} device(s))\n"  
            if verbose:  
                LOGGER.info(s if newline else s.rstrip())  
            return torch.device(f"npu:{npu_device}")  
              
        except ImportError:  
            raise ValueError("NPU device requested but torch_npu is not installed. Install torch_npu to use NPU devices.")  
        except Exception as e:  
            raise ValueError(f"NPU device error: {e}")  
